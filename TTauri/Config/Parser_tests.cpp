...
TEST(TTauriConfigParser, Assignments) {
    try {
        ...
        ASSERT_EQ(o->string(), "{foo:\"Hello World\",bar:1}");
        ASSERT_EQ(o->execute().string(), "{bar:1,foo:\"Hello World\"}");
...
TEST(TTauriConfigParser, Integers) {
    try {
        ...
        ASSERT_EQ(o->string(), "{a:0,b:1,c:10,d:2,e:8,f:10,g:16,h:0,i:-1,j:-10,k:-2,l:-8,m:-10,n:-16,o:-10,p:-2,q:-8,r:-10,s:-16}");
        ASSERT_EQ(o->execute().string(), "{a:0,b:1,c:10,d:2,e:8,f:10,g:16,h:0,i:-1,j:-10,k:-2,l:-8,m:-10,n:-16,o:-10,p:-2,q:-8,r:-10,s:-16}");
...
TEST(TTauriConfigParser, Floats) {
    try {
        ...
        ASSERT_EQ(o->string(), "{a:0.,b:-0.,c:1.,d:-1.,e:0.,f:-0.,g:0.1,h:-0.1,i:0.,j:-0.,k:1.,l:-1.}");
        ASSERT_EQ(o->execute().string(), "{a:0.,b:-0.,c:1.,d:-1.,e:0.,f:-0.,g:0.1,h:-0.1,i:0.,j:-0.,k:1.,l:-1.}");
...
TEST(TTauriConfigParser, Colors) {
    try {
        ...
        ASSERT_EQ(o->string(), parse_r);

        auto exec_r = "{a:#012345ff,b:#6889abcd}"s;
        ASSERT_EQ(o->execute().string(), exec_r);
...
TEST(TTauriConfigParser, Booleans) {
    try {
        ...
        ASSERT_EQ(o->string(), "{a:true,b:false,c:null}");
        ASSERT_EQ(o->execute().string(), "{a:true,b:false,c:null}");
...
TEST(TTauriConfigParser, Arrays) {
    try {
        ...
        ASSERT_EQ(o->string(), "{foo:[],bar:[1],baz:[1,2],bob:[1,2],a[]:3}");
        ASSERT_EQ(o->execute().string(), "{a:[3],bar:[1],baz:[1,2],bob:[1,2],foo:[]}");
...
TEST(TTauriConfigParser, Objects) {
    try {
        ...
        ASSERT_EQ(o->string(), "{foo:{a:1,\"b\":2},bar.baz:5,[hello],world:\"World\",[z],w:3}");
        ASSERT_EQ(o->execute().string(), "{bar:{baz:5},foo:{a:1,b:2},hello:{world:\"World\"},z:{w:3}}");
...
TEST(TTauriConfigParser, JSON) {
    try {
        ...
        ASSERT_EQ(o->string(), "{\"a\":1,\"b\":\"foo\",\"c\":1.1,\"d\":[1,2,3],\"e\":{\"a\":1,\"b\":1.1}}");
        ASSERT_EQ(o->execute().string(), "{a:1,b:\"foo\",c:1.1,d:[1,2,3],e:{a:1,b:1.1}}");
