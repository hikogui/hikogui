...
void my_preferences_window_controller::init_theme_tab(tt::grid_widget &grid) noexcept
{
    using namespace tt;

    {
        auto &theme_book = *grid.window.gui.theme_book;
        auto proxy = _theme_list.get();
    ...
    grid.make_widget<selection_widget>("B1", _theme_list, grid.window.gui.selected_theme);
}
...
void my_preferences_window_controller::init(tt::gui_window &self) noexcept
{
    ...
    self.toolbar().make_widget<toolbar_tab_button_widget>(label{elusive_icon::Key, l10n("License")}, tab_index, 1);
    self.toolbar().make_widget<toolbar_tab_button_widget>(label{elusive_icon::Brush, l10n("Theme")}, tab_index, 2);
    ...
    init_theme_tab(tabs.make_widget<grid_widget>(2));
}
