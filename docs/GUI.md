...
member function of a global object that can be acessed using
...
system is done from the same thread, called the gui\_thread.
...
the system gui\_loop, which will monitor keyboard, mouse events
...
of `tt::grid_layout_delegate`. It is mosty used to
