...
static void base93_number_to_bytes(bstring &message, ubig128 number, size_t nr_digits)
{
    ...
    ttlet nr_bytes = base93_nr_digits_to_nr_bytes(nr_digits);
...
static std::string base93_number_to_characters(ubig128 number, size_t nr_bytes) noexcept
{
    ...
    ttlet nr_digits = base93_nr_bytes_to_nr_digits(nr_bytes);
    auto r = std::string(nr_digits, '!');

    for (ssize_t i = nr_digits - 1; i >= 0; i--) {
        ttlet [quotient, remainder] = div(number, 93, oneOver93);
...
{
    ttlet number = base93_bytes_to_number(bytes);
    ttlet number_with_crc = base93_add_crc(number);
...
std::string base93_encode(bstring_view message) noexcept
{
    ...
    ttlet totalNumbers = (message.size() + 9) / 10;
    ttlet totalDigits = totalNumbers * 13;
    ttlet totalLineSeperators = (totalDigits + 5 + maximumLineLength) / maximumLineLength;
    ttlet totalCharacters = totalDigits + totalLineSeperators + 5;
    ...
    while (offset < message.size()) {
        ttlet nr_bytes = std::min(static_cast<size_t>(10), message.size() - offset);

        ttlet digits = base93_encode_bytes(message.substr(offset, nr_bytes));

        ttlet splitPosition = std::min(digits.size(), maximumLineLength - currentLineLength);
        ...
        case state_t::Decoding:
            if (nr_digits == 13 || c == '~') {
                if (!base93_crc_check(number)) {
                    ttlet [line, column] = count_line_and_columns(str.begin(), str.begin() + offset-1);
