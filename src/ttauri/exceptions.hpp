...
public:
    ...
    {
        return fmt::format("{}: {}. {}", name(), _message, error_info_string());
    }
...
public:
    ...
    template<typename Fmt, typename... Args>
    sub_error(Fmt const &fmt, Args &&... args) noexcept : error(fmt, std::forward<Args>(args)...)
    {
    }
    ...
    std::string error_info_string() const noexcept override
    {
        ...
        for (size_t i = 0; i < error_info.size(); i++) {
            ...
            r += fmt::format("{}={}", tag_name(error_info.get_tag(i)), error_info[i].repr());
        }
        return r;
    }
};
...
#define parse_assert(x) \
    if (!(x)) { \
        TTAURI_THROW(parse_error("{}", #x)); \
    }
#define parse_assert2(x, ...) \
    if (!(x)) { \
        TTAURI_THROW(parse_error(__VA_ARGS__)); \
    }

#define hresult_assert_or_throw(x) \
    ([](HRESULT result) { \
        if (FAILED(result)) { \
            [[unlikely]] TTAURI_THROW(io_error("Call to '{}' failed with {:08x}", #x, result)); \
        } \
        return result; \
    }(x))
