...
{
    auto tmp = new audio_system_aggregate(std::make_shared<audio_system_delegate>());
    tmp->init();
...
audio_system::audio_system(std::shared_ptr<audio_system_delegate> delegate) : _delegate(std::move(delegate)) {}
