...
struct veer_node {
    using statement_vector = typename std::vector<std::unique_ptr<veer_node>>;

    parse_location location;

    veer_node(parse_location location) :
    ...
    */
    [[nodiscard]] virtual bool append(std::unique_ptr<veer_node> x) noexcept { return false; }
    ...
    * @return datum::undefined when the veer_node generated textual data into context.output_text.
    ...
    */
    ...
    [[nodiscard]] friend std::string to_string(veer_node const &lhs) noexcept {
        return lhs.string();
    }

    friend std::ostream &operator<<(std::ostream &lhs, veer_node const &rhs) {
        return lhs << to_string(rhs);
    }

    static void append_child(statement_vector &children, std::unique_ptr<veer_node> new_child) {
