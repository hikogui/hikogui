...
struct veer_parse_context {
    using statement_stack_type = std::vector<std::unique_ptr<veer_node>>;
    ...
    */
    ...
    veer_parse_context() = delete;
    veer_parse_context(veer_parse_context const &other) = delete;
    veer_parse_context &operator=(veer_parse_context const &other) = delete;
    veer_parse_context(veer_parse_context &&other) = delete;
    veer_parse_context &operator=(veer_parse_context &&other) = delete;
    ~veer_parse_context() = default;

    veer_parse_context(URL const &url, const_iterator first, const_iterator last);
    ...
    veer_parse_context& operator++() noexcept {
        tt_assume(!atEOF());
        location += *index;
        ++index;
        return *this;
    ...
    veer_parse_context& operator+=(ssize_t x) noexcept {
    ...
    [[nodiscard]] bool append(std::unique_ptr<veer_node> x) noexcept;
