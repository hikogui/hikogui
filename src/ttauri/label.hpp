...
template<>
class observable_value<label> final : public observable_base<label> {
public:
    using super = observable_base<label>;
    ...
    observable_value(label const &value) noexcept : super(), _value(value)
    {
        _language_list_callback = language::preferred_languages.subscribe([this](auto...) {
            notify({}, load());
        });
    ...
    label load() const noexcept override
    {
        ttlet lock = std::scoped_lock(this->_mutex);
        return _value;
    }

    bool store(label const &new_value) noexcept override
    {
        this->_mutex.lock();

        ttlet old_value = _value;
        if constexpr (std::equality_comparable<label>) {
