...
static void inflate_copy_block(std::span<std::byte const> bytes, ssize_t &bit_offset, ssize_t max_size, bstring &r)
{
    ...
    parse_assert2((offset + LEN->value()) <= std::ssize(bytes), "input buffer overrun");
    parse_assert2((std::ssize(r) + LEN->value()) <= max_size, "output buffer overrun");
    ...
    bstring &r)
{
    while (true) {
        ...
        parse_assert2(((bit_offset + 27) >> 3) <= std::ssize(bytes), "Input buffer overrun");

        auto literal_symbol = literal_tree.get_symbol(bytes, bit_offset);

        if (literal_symbol <= 255) {
            parse_assert2(std::ssize(r) < max_size, "Output buffer overrun");
            ...
            parse_assert2(std::ssize(r) + length <= max_size, "Output buffer overrun");

            // Test only every get_symbol, the trailer is at least 32 bits (Checksum)
            // - 15 bits maximum huffman code.
            // -  7 bits rounding up to byte.
            parse_assert2(((bit_offset + 22) >> 3) <= std::ssize(bytes), "Input buffer overrun");
            ...
            // -  7 bits rounding up to byte.
            parse_assert2(((bit_offset + 20) >> 3) <= std::ssize(bytes), "Input buffer overrun");
            auto distance = inflate_decode_distance(bytes, bit_offset, distance_symbol);

            parse_assert2(distance <= std::ssize(r), "Distance beyond start of decompressed data");
...
[[nodiscard]] static huffman_tree<int16_t> inflate_code_lengths(std::span<std::byte const> bytes, ssize_t &bit_offset, int nr_symbols)
{
    ...
    parse_assert2(((bit_offset + (3 * static_cast<ssize_t>(nr_symbols)) + 7) >> 3) <= std::ssize(bytes), "Input buffer overrun");
    ...
    huffman_tree<int16_t> const &code_length_tree)
{
    ...
    while (std::ssize(r) < nr_symbols) {
        ...
        // -  7 bits extra length.
        // -  7 bits rounding up to byte.
        parse_assert2(((bit_offset + 21) >> 3) <= std::ssize(bytes), "Input buffer overrun");
...
bstring inflate(std::span<std::byte const> bytes, ssize_t &offset, ssize_t max_size)
{
    ...
    do {
        ...
        // - 3 bits header
        // - 7 bits rounding up to byte.
        parse_assert2(((bit_offset + 10) >> 3) <= std::ssize(bytes), "Input buffer overrun");
