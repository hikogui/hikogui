...
#include "ttauri/stencil/stencil.hpp"
...
TEST(Stencil, Text) {
    std::unique_ptr<stencil_node> t;
    std::string result;

    ASSERT_NO_THROW(t = parse_stencil(URL("none:"), ""));
    ASSERT_EQ(to_string(*t), "<top >");
    ASSERT_NO_THROW(result = t->evaluate_output());
    ASSERT_EQ(result, "");

    ASSERT_NO_THROW(t = parse_stencil(URL("none:"), "foo"));
    ASSERT_EQ(to_string(*t), "<top <text foo>>");
    ASSERT_NO_THROW(result = t->evaluate_output());
    ASSERT_EQ(result, "foo");
}

TEST(Stencil, Placeholder) {
    std::unique_ptr<stencil_node> t;
    std::string result;

    ASSERT_NO_THROW(t = parse_stencil(URL("none:"), "foo${42}bar"));
...
TEST(Stencil, FunctionReplaceAndSuper) {
    std::unique_ptr<stencil_node> t;
    std::string result;
...
TEST(Stencil, Include) {
    std::unique_ptr<stencil_node> t;

    ASSERT_NO_THROW(t = parse_stencil(URL("file:includer.ttt")));
