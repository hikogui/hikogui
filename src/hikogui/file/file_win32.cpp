...
struct file_win32 final : detail::file_impl {
    ...
    file_win32(std::filesystem::path path, access_mode access_mode) : detail::file_impl(std::move(path), access_mode)
    {
        ...
        } else if (any(access_mode & access_mode::write)) {
            desired_access = GENERIC_WRITE;
        } else {
            throw io_error(std::format("{}: Invalid AccessMode; expecting Readable and/or Writeable.", this->path.string()));
        }

        DWORD share_mode;
        if (any(access_mode & access_mode::write_lock)) {
            share_mode = 0;
        } else if (any(access_mode & access_mode::read_lock)) {
        ...
        if (any(access_mode & access_mode::create) and any(access_mode & access_mode::open)) {
            if (any(access_mode & access_mode::truncate)) {
                creation_disposition = CREATE_ALWAYS;
            } else {
                creation_disposition = OPEN_ALWAYS;
            }

        } else if (any(access_mode & access_mode::create)) {
            creation_disposition = CREATE_NEW;

        } else if (any(access_mode & access_mode::open)) {
            if (any(access_mode & access_mode::truncate)) {
            ...
            throw io_error(std::format("{}: Invalid AccessMode; expecting CreateFile and/or OpenFile.", this->path.string()));
        }

        DWORD flags_and_attributes = 0;
        if (any(access_mode & access_mode::random)) {
            flags_and_attributes |= FILE_FLAG_RANDOM_ACCESS;
        }
        if (any(access_mode & access_mode::sequential)) {
            flags_and_attributes |= FILE_FLAG_SEQUENTIAL_SCAN;
        }
        if (any(access_mode & access_mode::write_through)) {
            flags_and_attributes |= FILE_FLAG_WRITE_THROUGH;
        }

        if (any(access_mode & access_mode::rename)) {
            desired_access |= DELETE;
        }

        hilet file_name = this->path.native();
        if ((_file_handle =
        ...
        if (any(access_mode & access_mode::create_directories) and error == ERROR_PATH_NOT_FOUND and
            ...
            auto directory = this->path;
            directory.remove_filename();
            std::filesystem::create_directories(directory);

            if ((_file_handle =
                     CreateFileW(file_name.data(), desired_access, share_mode, NULL, creationDisposition, flags_and_attributes, NULL)) !=
                INVALID_HANDLE_VALUE) {
                return;
            }
        }
        throw io_error(std::format("{}: Could not open file, '{}'", this->path.string(), get_last_error_message()));
    }
    ...
    void rename(std::filesystem::path const &destination, bool overwrite_existing) override
    {
        ...
        if (!r) {
            throw io_error(
                std::format("{}: Could not rename file to {}. '{}'", get_last_error_message(), destination.string()));
        }
    }

    std::size_t file::write(void const *data, std::size_t size) override
    {
        hi_axiom(_file_handle != INVALID_HANDLE_VALUE);

        auto total_written = 0_uz;
        while (size != 0) {
    ...
    std::size_t read(void *data, std::size_t size) override
    {
        ...
        while (size) {
            ...
            if (!ReadFile(_file_handle, data, to_read_size, &has_read, nullptr)) {
