...
#define HI_ASSERT_SIMD_EQ(val1, val2) ASSERT_PRED_FORMAT2(native_u32x4_tests::CmpHelperEQ, val1, val2)
#define HI_ASSERT_SIMD_NE(val1, val2) ASSERT_PRED_FORMAT2(native_u32x4_tests::CmpHelperNE, val1, val2)

using S = hi::native_simd<uint32_t,4>;
...
{
    HI_ASSERT_SIMD_EQ(+S(0, 2, 3, 0x7fff'ffff), S(0, 2, 3, 0x7fff'ffff));
    HI_ASSERT_SIMD_EQ(S(0, 2, 3, 0x7fff'ffff) + S(1, 4, 0xffff'fffd, 2), S(1, 6, 0, 0x8000'0001));
    HI_ASSERT_SIMD_EQ(S(0, 2, 3, 0x8000'0001) - S(1, 4, 0xffff'fffd, 2), S(0xffff'ffff, 0xffff'fffe, 6, 0x7fff'ffff));
    HI_ASSERT_SIMD_EQ(S(0, 2, 3, 0x7fff'ffff) * S(1, 4, 0xffff'fffd, 2), S(0, 8, 0xffff'fff7, 0xffff'fffe));

    HI_ASSERT_SIMD_EQ(min(S(0, 2, 0, 0x7fff'ffff), S(1, 0, 0xffff'fffd, 1)), S(0, 0, 0, 1));
    HI_ASSERT_SIMD_EQ(max(S(0, 2, 0, 0x7fff'ffff), S(1, 0, 0xffff'fffd, 1)), S(1, 2, 0xffff'fffd, 0x7fff'ffff));
}

TEST(native_u32x4, bit_wise)
{
    HI_ASSERT_SIMD_EQ(S(0, 2, 0x8000'0000, 42) >> 1, S(0, 1, 0x4000'0000, 21));
    HI_ASSERT_SIMD_EQ(S(0, 2, 0x8000'0001, 42) << 1, S(0, 4, 2, 84));
...
TEST(native_u32x4, horizontal)
{
    ...
    HI_ASSERT_SIMD_EQ(horizontal_sub(S(42, 3, 34, 5), S(2, 13, 24, 15)), S(39, 29, 0xffff'fff5, 9));
#endif
