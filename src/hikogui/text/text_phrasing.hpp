...
enum class text_phrasing_mask : uint16_t {
    regular = 1 << std::to_underlying(text_phrasing::regular),
    emphesis = 1 << std::to_underlying(text_phrasing::emphesis),
    strong = 1 << std::to_underlying(text_phrasing::strong),
    code = 1 << std::to_underlying(text_phrasing::code),
    abbreviation = 1 << std::to_underlying(text_phrasing::abbreviation),
    bold = 1 << std::to_underlying(text_phrasing::bold),
    italic = 1 << std::to_underlying(text_phrasing::italic),
    citation = 1 << std::to_underlying(text_phrasing::citation),
    keyboard = 1 << std::to_underlying(text_phrasing::keyboard),
    mark = 1 << std::to_underlying(text_phrasing::mark),
    math = 1 << std::to_underlying(text_phrasing::math),
    example = 1 << std::to_underlying(text_phrasing::example),
    unarticulated = 1 << std::to_underlying(text_phrasing::unarticulated),
...
{
    hi_axiom(std::to_underlying(rhs) < sizeof(text_phrasing_mask) * CHAR_BIT);
    return static_cast<text_phrasing_mask>(1 << std::to_underlying(rhs));
}

[[nodiscard]] constexpr text_phrasing_mask operator&(text_phrasing_mask const& lhs, text_phrasing_mask const& rhs) noexcept
{
    return static_cast<text_phrasing_mask>(std::to_underlying(lhs) & std::to_underlying(rhs));
}
