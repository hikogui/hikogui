...
/** A notifier which holds a set of functions.
 *
 * @tparam Result The result type of the functions.
...
public:
    ...
    /** The result type of the functions.
     */
    ...
    /** Add a function to the notifier.
     * Ownership of the function belongs with the caller of `subscribe()`. The
     * caller will receive a token, a RAII object that will unsubscribe the function
     * when the token is destroyed.
     *
     * @param function A function.
     * @return A RAII object which when destroyed will unsubscribe the function.
     */
    [[nodiscard]] token_type subscribe(std::invocable<Args...> auto&& function) noexcept
    {
        auto token = std::make_shared<function_type>(hi_forward(function));
        hilet lock = std::scoped_lock(_mutex);
        _functions.emplace_back(token);
        return token;
    }

    /** Post the subscribed functions on the local-thread's event loop with the given arguments.
     *
     * @param args The arguments to pass with the invocation of the function
     */
    void post(Args const&...args) const noexcept requires(std::is_same_v<result_type, void>)
    {
        for_each([&](token_type token) {
    ...
    /** Post the subscribed functions on the main-thread's event loop with the given arguments.
     *
     * @param args The arguments to pass with the invocation of the function
     */
...
private:
     ...
     */
    mutable std::vector<weak_token_type> _functions;

    void for_each(auto&& func) const noexcept
    {
        hilet lock = std::scoped_lock(_mutex);

        auto it = _functions.begin();
        auto last = _functions.end();
