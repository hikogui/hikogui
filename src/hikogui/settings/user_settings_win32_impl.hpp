...
[[nodiscard]] inline std::expected<std::string, std::error_code> get_user_setting_string(std::string_view name) noexcept
{
    ...
    if (hilet value = win32_RegGetValue<std::string>(registry_key::current_user, user_setting_registry_path(), name)) {
        return *value;
    ...
    if (hilet value = win32_RegGetValue<std::string>(registry_key::local_machine, user_setting_registry_path(), name)) {
        return *value;
    } else {
        return std::unexpected{std::error_code{value.error()}};
    }
...
{
    // First check the registry of the current-user.
    if (hilet value = win32_RegGetValue<long long>(registry_key::current_user, user_setting_registry_path(), name)) {
        return *value;
    } else if (value.error() != win32_error::file_not_found) {
        return std::unexpected{std::error_code{value.error()}};
    }
    ...
    // Now check the registry for the local-machine.
    // These are settings that where made by the Administrator of the machine.
    if (hilet value = win32_RegGetValue<long long>(registry_key::local_machine, user_setting_registry_path(), name)) {
        return *value;
    } else {
        return std::unexpected{std::error_code{value.error()}};
    }
...
{
    return win32_RegSetKeyValue(registry_key::current_user, user_setting_registry_path(), name, value);
}

inline std::error_code set_user_setting(std::string_view name, long long value) noexcept
{
    return win32_RegSetKeyValue(registry_key::current_user, user_setting_registry_path(), name, narrow_cast<uint32_t>(value));
}

inline std::error_code delete_user_setting(std::string_view name) noexcept
{
    return win32_RegDeleteKeyValue(registry_key::current_user, user_setting_registry_path(), name);
}

inline std::error_code delete_user_settings() noexcept
{
    return win32_RegDeleteKey(registry_key::current_user, user_setting_registry_path());
}
