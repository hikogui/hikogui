...
void device_shared::clear_staging_between_border_and_upload(aarectangle border_rectangle, aarectangle upload_rectangle) noexcept
{
    ...
    hilet border_top = ceil_cast<std::size_t>(border_rectangle.top());
    hilet border_right = ceil_cast<std::size_t>(border_rectangle.right());
    hilet upload_top = ceil_cast<std::size_t>(upload_rectangle.top());
    hilet upload_right = ceil_cast<std::size_t>(upload_rectangle.right());
...
void device_shared::update_atlas_with_staging_pixmap(paged_image const &image) noexcept
{
    ...
    for (std::size_t index = 0; index < size(image.pages); index++) {
        ...
        hilet src_x = floor_cast<int32_t>(src_position.x() - 1);
        hilet src_y = floor_cast<int32_t>(src_position.y() - 1);
        hilet dst_x = floor_cast<int32_t>(dst_position.x() - 1);
        hilet dst_y = floor_cast<int32_t>(dst_position.y() - 1);
