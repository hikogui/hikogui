...
namespace hi::inline v1::gfx_pipeline_alpha {

/* Do not blend, simply use just the alpha channel and overwrite the pixels in the color attachment directly.
 */
std::vector<vk::PipelineColorBlendAttachmentState> gfx_pipeline_alpha::getPipelineColorBlendAttachmentStates() const
{
    return {
...
void gfx_pipeline_alpha::draw_in_command_buffer(vk::CommandBuffer commandBuffer, draw_context const& context)
{
...
std::vector<vk::PipelineShaderStageCreateInfo> gfx_pipeline_alpha::createShaderStages() const
{
    hi_axiom_not_null(device());
    return device()->alpha_pipeline->shaderStages;
}

std::vector<vk::DescriptorSetLayoutBinding> gfx_pipeline_alpha::createDescriptorSetLayoutBindings() const
{
    return {};
}

std::vector<vk::WriteDescriptorSet> gfx_pipeline_alpha::createWriteDescriptorSet() const
{
    return {};
}

size_t gfx_pipeline_alpha::getDescriptorSetVersion() const
{
    return 0;
}

std::vector<vk::PushConstantRange> gfx_pipeline_alpha::createPushConstantRanges() const
{
    return push_constants::pushConstantRanges();
}

vk::VertexInputBindingDescription gfx_pipeline_alpha::createVertexInputBindingDescription() const
{
    return vertex::inputBindingDescription();
}

std::vector<vk::VertexInputAttributeDescription> gfx_pipeline_alpha::createVertexInputAttributeDescriptions() const
{
    return vertex::inputAttributeDescriptions();
}

void gfx_pipeline_alpha::build_vertex_buffers()
{
...
void gfx_pipeline_alpha::teardown_vertex_buffers()
{
    hi_axiom_not_null(device());
