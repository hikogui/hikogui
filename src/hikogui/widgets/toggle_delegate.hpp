...
/** @file widgets/toggle_delegate.hpp Defines toggle_delegate and some default toggle_delegate delegates.
...
public:
    ...
    default_toggle_delegate(
        ...
        : default_toggle_delegate(hi_forward(value), hi_forward(on_value), default_values<value_type>::off)
    {
    }
    ...
    default_toggle_delegate(forward_of<observer<value_type>> auto&& value) noexcept
        requires default_values_v<value_type>
        : default_toggle_delegate(hi_forward(value), default_values<value_type>::on, default_values<value_type>::off)
    {
    }
 ...
 */
[[nodiscard]] std::shared_ptr<toggle_delegate> make_default_toggle_delegate(auto&& value, auto&&...args) noexcept
    requires requires {
                 default_toggle_delegate<observer_decay_t<decltype(value)>>{hi_forward(value), hi_forward(args)...};
             }
{
    return std::make_shared<default_toggle_delegate<observer_decay_t<decltype(value)>>>(
