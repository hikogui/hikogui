...
[[nodiscard]] box_constraints text_field_widget::constraints(constraints_context const& context) noexcept
{
    hi_assert_not_null(delegate);
    ...
    _scroll_constraints = _scroll_widget->constraints(context);
    ...
    if (_error_label->empty()) {
        ...
        _error_label_constraints = _error_label_widget->constraints(context);
