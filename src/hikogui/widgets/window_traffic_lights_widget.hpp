...
public:
    ...
    [[nodiscard]] box_constraints update_constraints() noexcept override
    {
        if (operating_system::current == operating_system::windows) {
            ...
            hilet size = extent2{theme_size * 3, theme_size};
            ...
            hilet size = extent2{theme_size * 3 + 2 * margin + 2 * spacing, theme_size + 2 * spacing};
            return {size, size, size};
    ...
    void set_layout(widget_layout const& context) noexcept override
    {
        if (compare_store(layout, context)) {
            ...
            if (operating_system::current == operating_system::windows) {
                closeRectangle =
                    aarectangle{point2(extent.width() * 2 / 3, y), extent2{extent.width() * 1 / 3, extent.height()}};

                maximizeRectangle =
                    aarectangle{point2(extent.width() * 1 / 3, y), extent2{extent.width() * 1 / 3, extent.height()}};

                minimizeRectangle = aarectangle{point2(0, y), extent2{extent.width() * 1 / 3, extent.height()}};
                ...
                closeRectangle = aarectangle{point2{margin, std::floor((extent.height() - size) / 2.0f)}, extent2{size, size}};

                minimizeRectangle =
                    aarectangle{point2{margin + size + spacing, std::floor((extent.height() - size) / 2.0f)}, extent2{size, size}};

                maximizeRectangle = aarectangle{
                    point2{margin + size + spacing + size + spacing, std::floor((extent.height() - size) / 2.0f)}, extent2{size, size}};
            } else {
                hi_no_default();
            }
            ...
            hilet closeWindowGlyphBB = closeWindowGlyph.get_bounding_rectangle() * glyph_size;
            hilet minimizeWindowGlyphBB = minimizeWindowGlyph.get_bounding_rectangle() * glyph_size;
            hilet maximizeWindowGlyphBB = maximizeWindowGlyph.get_bounding_rectangle() * glyph_size;
            hilet restoreWindowGlyphBB = restoreWindowGlyph.get_bounding_rectangle() * glyph_size;
