...
[[nodiscard]] box_constraints checkbox_widget::update_constraints() noexcept
{
    ...
    _button_size = tv<"checkbox.button.size", extent2i>{}(scale);
    hilet extra_size = extent2i{tv<"checkbox.button.margin", int>{}(scale) + _button_size.width(), 0};

    auto constraints = max(_label_constraints + extra_size, _button_size);
    constraints.margins = tv<"checkbox.button.margin", marginsi>{}(scale);
...
void checkbox_widget::set_layout(widget_layout const& context) noexcept
{
    if (compare_store(_layout, context)) {
        ...
        hilet label_width = context.width() - (_button_rectangle.width() + tv<"checkbox.button.margin", int>{}(scale));
        if (alignment_ == horizontal_alignment::left) {
            hilet label_left = _button_rectangle.right() + tv<"checkbox.button.margin", int>(scale);
            ...
            _on_label_shape = _off_label_shape = _other_label_shape =
                box_shape(_label_constraints, label_rectangle, tv<"checkbox.button.baseline", int>{}(scale));
        ...
        hilet check_glyph_bb =
            narrow_cast<aarectanglei>(_check_glyph.get_bounding_box() * tv<"checkbox.button.icon.size", float>{}(scale));
        ...
        hilet minus_glyph_bb =
            narrow_cast<aarectanglei>(_minus_glyph.get_bounding_box() * tv<"checkbox.button.icon.size", float>{}(scale));
...
void checkbox_widget::draw_check_box(draw_context const& context) noexcept
{
    context.draw_box(
        ...
        background_color(),
        focus_color(),
        tv<"checkbox.button.border.width", float>{}(scale),
        border_side::inside);
}
