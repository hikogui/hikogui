...
[[nodiscard]] __m128 _mm_swizzle_ps(__m128 const& value) noexcept
{
    ...
    hilet swizzled = [&] {
        ...
        if constexpr (permute_mask != 0b11'10'01'00) {
            return _mm_permute_ps(value, permute_mask);
        } else {
            return value;
        }
    }();
    ...
    hilet numbers = [&] {
        if constexpr (not_one_mask == 0b0000) {
            ...
            return _mm_insert_ps(_1111, _1111, not_one_mask);
        }
    }();
    ...
    if constexpr (number_mask == 0b0000) {
        ...
        return _mm_insert_ps(swizzled, swizzled, number_mask);
    } else {
        return _mm_blend_ps(swizzled, numbers, number_mask);
    }
}
...
[[nodiscard]] __m128i _mm_swizzle_epi32(__m128i const& value) noexcept
{
    ...
    hilet swizzled = [&] {
        // Clang is able to optimize these intrinsics, MSVC is not.
        if constexpr (permute_mask != 0b11'10'01'00) {
            return _mm_castps_si128(_mm_permute_ps(_mm_castsi128_ps(value), permute_mask));
        } else {
            return value;
        }
    }();
    ...
    hilet numbers = [&] {
        if constexpr (not_one_mask == 0b0000) {
            ...
            hilet _1111 = _mm_castsi128_ps(_mm_set1_epi32(1));
            return _mm_castps_si128(_mm_insert_ps(_1111, _1111, not_one_mask));
        }
    }();
    ...
    if constexpr (number_mask == 0b0000) {
        ...
        return _mm_castps_si128(_mm_insert_ps(_mm_castsi128_ps(swizzled), _mm_castsi128_ps(swizzled), number_mask));
    } else {
        return _mm_castps_si128(_mm_blend_ps(_mm_castsi128_ps(swizzled), _mm_castsi128_ps(numbers), number_mask));
    }
}
