...
/** Find all widths where the height of the text changes.
 *
 * @return A list of widths, the lowest width at each text height change.
 */
[[nodiscard]] constexpr std::vector<std::pair<float, std::vector<size_t>>>
unicode_line_break_text_widths(unicode_break_vector const& opportunities, std::vector<float> const& widths) noexcept
{
    struct entry_type {
        std::vector<size_t> min_lines;
        std::vector<size_t> max_lines;
        float min_width;
        float max_width;
    };
    ...
    auto r = std::vector<std::pair<float, std::vector<size_t>>>{};
    r.emplace_back(min_width, min_lines);
    if (min_lines.size() != max_lines.size()) {
        r.emplace_back(max_width, max_lines);
        stack.emplace_back(std::move(min_lines), std::move(max_lines), min_width, max_width);
    }
    ...
    while (not stack.empty()) {
        ...
        if (entry.max_lines.size() > entry.min_lines.size() + 1 and entry.max_width >= entry.min_width + 2.0f) {
            ...
            if (split_lines.size() == entry.min_lines.size()) {
                // We didn't find a proper split, need to try the upper half. Use `half_width` to split right down the middle.
                stack.emplace_back(std::move(split_lines), std::move(entry.max_lines), half_width, entry.max_width);

            } else if (split_lines.size() == entry.max_lines.size()) {
                // We didn't find a proper split, need to try the lower half. Use `half_width` to split right down the middle.
                stack.emplace_back(std::move(entry.min_lines), std::move(split_lines), entry.min_width, half_width);

            } else {
                // Split through the middle, use the split_width for faster searching.
                r.emplace_back(split_width, split_lines);
                stack.emplace_back(std::move(entry.min_lines), split_lines, entry.min_width, split_width);
                stack.emplace_back(std::move(split_lines), std::move(entry.max_lines), split_width, entry.max_width);
            }
        }
    }
