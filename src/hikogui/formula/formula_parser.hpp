...
hi_export_module(hikogui.formula.formula_parser);
...
[[nodiscard]] hi_inline std::unique_ptr<formula_node>
 ...
 * Parses an formula until EOF, ')', ',', '}'
 */
[[nodiscard]] hi_inline std::unique_ptr<formula_node> parse_formula_without_post_processing(std::string_view text)
{
 ...
 * Parses an formula until EOF, ')', ',', '}'
 */
[[nodiscard]] hi_inline std::unique_ptr<formula_node> parse_formula(std::string_view text, formula_post_process_context post_process_context = {})
{
