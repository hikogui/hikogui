 ...
 */
export [[nodiscard]] generator<std::filesystem::path> glob(glob_pattern pattern) noexcept
{
    ...
    auto first = last;
    try {
        first = std::filesystem::recursive_directory_iterator(path);
    } catch (...) {
        co_return;
    }

    for (auto it = first; it != last; ++it) {
        std::filesystem::path const *iterated_path = nullptr;
        try {
            iterated_path = std::addressof(it->path());
        } catch (...) {
            continue;
        }
 ...
 * @return a generator yielding paths to objects on the filesystem that match the pattern.
 */
export [[nodiscard]] generator<std::filesystem::path> glob(std::string_view pattern) noexcept
{
 ...
 * @return a generator yielding paths to objects on the filesystem that match the pattern.
 */
export [[nodiscard]] generator<std::filesystem::path> glob(std::string pattern) noexcept
{
 ...
 * @return a generator yielding paths to objects on the filesystem that match the pattern.
 */
export [[nodiscard]] generator<std::filesystem::path> glob(char const *pattern) noexcept
{
    return glob(glob_pattern{pattern});
}
 ...
 * @return a generator yielding paths to objects on the filesystem that match the pattern.
 */
export [[nodiscard]] generator<std::filesystem::path> glob(std::filesystem::path pattern) noexcept
{
    return glob(glob_pattern{std::move(pattern)});
}
...
[[nodiscard]] generator<std::filesystem::path> glob(Locations&& locations, std::filesystem::path ref) noexcept
{
 ...
 */
export template<path_range Locations>
[[nodiscard]] generator<std::filesystem::path> glob(Locations&& locations, std::string_view ref) noexcept
{
 ...
 */
export template<path_range Locations>
[[nodiscard]] generator<std::filesystem::path> glob(Locations&& locations, std::string ref) noexcept
{
    return glob(std::forward<Locations>(locations), std::filesystem::path{ref});
}
 ...
 */
export template<path_range Locations>
[[nodiscard]] generator<std::filesystem::path> glob(Locations&& locations, char const *ref) noexcept
{
    return glob(std::forward<Locations>(locations), std::filesystem::path{ref});
}
