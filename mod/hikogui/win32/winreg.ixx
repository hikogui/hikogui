...
export module hikogui_win32_winreg;
 ...
 */
[[nodiscard]] win32_error win32_RegSetKeyValue(HKEY key, std::string_view path, std::string_view name, uint32_t value)
{
    hilet wpath = win32_MultiByteToWideChar(path);
    if (not wpath) {
        return wpath.error();
 ...
 * @return 0 on success, or error code on failure.
 */
[[nodiscard]] win32_error win32_RegSetKeyValue(HKEY key, std::string_view path, std::string_view name, std::string_view value)
{
    hilet wpath = win32_MultiByteToWideChar(path);
    if (not wpath) {
        return wpath.error();
 ...
 */
[[nodiscard]] std::expected<void, win32_error> win32_RegGetValue_void(HKEY key, std::string_view path, std::string_view name)
{
    hilet wpath = win32_MultiByteToWideChar(path);
    if (not wpath) {
 ...
 */
[[nodiscard]] std::expected<uint32_t, win32_error> win32_RegGetValue_dword(HKEY key, std::string_view path, std::string_view name) noexcept
{
    hilet wpath = win32_MultiByteToWideChar(path);
    if (not wpath) {
        return std::unexpected{wpath.error()};
 ...
 * @return value, or win32_error::file_not_found if entry was not found, otherwise an error.
 */
[[nodiscard]] std::expected<std::string, win32_error> win32_RegGetValue_string(HKEY key, std::string_view path, std::string_view name) noexcept
{
    hilet wpath = win32_MultiByteToWideChar(path);
    if (not wpath) {
        return std::unexpected{wpath.error()};
 ...
 */
[[nodiscard]] std::expected<std::vector<std::string>, win32_error>
...
template<>
[[nodiscard]] std::expected<void, win32_error> win32_RegGetValue(HKEY key, std::string_view path, std::string_view name)
{
    return win32_RegGetValue_void(key, path, name);
}

template<std::integral T>
[[nodiscard]] std::expected<T, win32_error> win32_RegGetValue(HKEY key, std::string_view path, std::string_view name)
{
...
template<>
[[nodiscard]] std::expected<std::string, win32_error> win32_RegGetValue(HKEY key, std::string_view path, std::string_view name)
{
