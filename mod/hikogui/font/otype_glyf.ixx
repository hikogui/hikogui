 ...
 */
[[nodiscard]] generator<otype_glyf_component> otype_glyf_get_compound(std::span<std::byte const> bytes, float em_scale)
{
    if (bytes.empty()) {
        // Empty glyphs have no path, and therefor an empty bounding rectangle.
        co_return;
 ...
 *
 */
[[nodiscard]] bool otype_glyf_is_compound(std::span<std::byte const> bytes)
{
    struct header_type {
        big_int16_buf_t num_contours;
        otype_fword_buf_t x_min;
 ...
 * @note Only call this function when `otype_glyf_is_compound() == false`.
 */
[[nodiscard]] aarectangle otype_glyf_get_bounding_box(std::span<std::byte const> bytes, float em_scale)
{
    struct header_type {
        big_int16_buf_t num_contours;
        otype_fword_buf_t x_min;
