...
export module hikogui_SIMD : float16_sse4_1;
...
{
    auto const f32_to_f16_constants = _mm_set_epi32(0, f32_to_f16_adjustment, f32_to_f16_infinite, f32_to_f16_lowest_normal - 1);

    // Convert the 16 bit values to 32 bit with leading zeros.
    auto u = _mm_unpacklo_epi16(value, _mm_setzero_si128()); // SSE2

    // Extract the sign bit.
    auto const sign = _mm_slli_epi32(_mm_srli_epi32(u, 15), 31); // SSE2
    ...
    auto const f32_to_f16_adjustment_ = _mm_shuffle_epi32(f32_to_f16_constants, 0b10'10'10'10); // SSE2
    u = _mm_add_epi32(u, f32_to_f16_adjustment_); // SSE2

    // Get a mask of '1' bits when the half-float would be normal or infinite.
    auto const f32_to_f16_lowest_normal_ = _mm_shuffle_epi32(f32_to_f16_constants, 0b00'00'00'00); // SSE2
    auto const is_normal = _mm_cmpgt_epi32(u, f32_to_f16_lowest_normal_); // SSE2
...
__m128i _mm_cvtps_ph_sse4_1(__m128 value) noexcept
{
    ...
    auto const sign = _mm_slli_epi32(_mm_srai_epi32(u, 31), 15); // SSE2
    ...
    // Get a mask of '1' bits when the half-float would be normal or infinite.
    auto const f32_to_f16_lowest_normal_ = _mm_shuffle_epi32(f32_to_f16_constants, 0b00'00'00'00); // SSE2
    auto const is_normal = _mm_cmpgt_epi32(u, f32_to_f16_lowest_normal_);

    // Clamp the floating point number to where the half-float would be infinite.
    auto const f32_to_f16_infinite_ = _mm_shuffle_epi32(f32_to_f16_constants, 0b01'01'01'01); // SSE2
