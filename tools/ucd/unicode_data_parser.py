...
def parse_unicode_data(filename):
    for columns in parse_ucd(filename):
        ...
        tmp = decomposition_type_mapping.split(">")
        if len(tmp) == 2:
            decomposition_type = tmp[0][1:]
            decomposition_mapping = [int(x, 16) for x in tmp[1]]
        else:
            decomposition_type = "canonical"
            decomposition_mapping = [int(x, 16) for x in tmp[0]]
