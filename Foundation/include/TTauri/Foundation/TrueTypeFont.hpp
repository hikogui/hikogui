...
private:
    ...
    */
     ...
     * called by loadGlyph()
     */
    bool getGlyphBytes(GlyphID glyph_id, gsl::span<std::byte const> &bytes) const noexcept;
     ...
     */
    bool loadCompoundGlyph(gsl::span<std::byte const> bytes, Path &glyph, GlyphID &metrics_glyph_id) const noexcept;
    ...
    *                          this value is only updated when the USE_MY_METRICS flag was set.
    */
    bool loadCompoundGlyphMetrics(gsl::span<std::byte const> bytes, GlyphID &metrics_glyph_id) const noexcept;
