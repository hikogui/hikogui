...
//force_inline bigint<T,N> bigint_reciprocal(bigint<T,O> const &divider);
...
struct bigint {
    ...
    */
    ...
    force_inline bigint() noexcept = default;
    force_inline ~bigint() = default;
    force_inline bigint(bigint const &) noexcept = default;
    force_inline bigint &operator=(bigint const &) noexcept = default;
    force_inline bigint(bigint &&) noexcept = default;
    force_inline bigint &operator=(bigint &&) noexcept = default;

    template<int R, std::enable_if_t<R < N, int> = 0>
    force_inline bigint(bigint<T,R> const &rhs) noexcept {
        int i = 0;
        for (; i != R; ++i) {
    ...
    force_inline bigint(R value) noexcept {
    ...
    template<typename R, std::enable_if_t<std::is_integral_v<R>, int> = 0>
    force_inline bigint &operator=(R value) noexcept {
        digits[0] = value;
        for (auto i = 1; i < nr_digits; i++) {
            digits[i] = 0;
        }
    ...
    force_inline explicit bigint(std::string_view str, int base=10) noexcept :
        bigint(0)
    {
    ...
    force_inline explicit operator unsigned long long () const noexcept { return static_cast<unsigned long long>(digits[0]); }
    force_inline explicit operator signed long long () const noexcept { return static_cast<signed long long>(digits[0]); }
    force_inline explicit operator unsigned long () const noexcept { return static_cast<unsigned long>(digits[0]); }
    force_inline explicit operator signed long () const noexcept { return static_cast<signed long>(digits[0]); }
    force_inline explicit operator unsigned int () const noexcept { return static_cast<unsigned int>(digits[0]); }
    force_inline explicit operator signed int () const noexcept { return static_cast<signed int>(digits[0]); }
    force_inline explicit operator unsigned short () const noexcept { return static_cast<unsigned short>(digits[0]); }
    force_inline explicit operator signed short () const noexcept { return static_cast<signed short>(digits[0]); }
    force_inline explicit operator unsigned char () const noexcept { return static_cast<unsigned char>(digits[0]); }
    force_inline explicit operator signed char () const noexcept { return static_cast<signed char>(digits[0]); }
    force_inline explicit operator char () const noexcept { return static_cast<char>(digits[0]); }
    ...
    template<int O>
    force_inline explicit operator bigint<T,O> () const noexcept {
        bigint<T,O> r;
    ...
    template<typename R, std::enable_if_t<std::is_integral_v<R>, int> = 0>
    force_inline bigint &operator<<=(R rhs) noexcept {
        bigint_shift_left(*this, *this, static_cast<int>(rhs));
        return *this;
    }

    template<typename R, std::enable_if_t<std::is_integral_v<R>, int> = 0>
    force_inline constexpr bigint &operator>>=(R rhs) noexcept {
        bigint_shift_right(*this, *this, static_cast<int>(rhs));
        return *this;
    }

    force_inline bigint &operator*=(bigint const &rhs) noexcept {
        auto r = bigint<T,N>{0};
    ...
    force_inline bigint crc(bigint const &rhs) noexcept {
        return bigint_crc(*this, rhs);
    }

    static force_inline bigint fromBigEndian(uint8_t const *data) noexcept {
        auto r = bigint{};
        for (int i = N - 1; i >= 0; i--) {
            digit_type d = 0;
    ...
    static force_inline bigint fromLittleEndian(uint8_t const *data) noexcept {
        auto r = bigint{};
        for (int i = 0; i < N; i++) {
            digit_type d = 0;
            for (ssize_t j = 0; j < sizeof(digit_type); j++) {
    ...
    static force_inline bigint fromBigEndian(void const *data) noexcept {
        return fromBigEndian(static_cast<uint8_t const *>(data));
    }

    static force_inline bigint fromLittleEndian(void const *data) noexcept {
        return fromLittleEndian(static_cast<uint8_t const *>(data));
    }

    [[nodiscard]] force_inline friend int bigint_compare(bigint const &lhs, bigint const &rhs) noexcept
    {
        for (int i = N - 1; i >= 0; --i) {
    ...
    force_inline friend void bigint_invert(bigint &o, bigint const &rhs) noexcept
    {
        for (auto i = 0; i < N; i++) {
    ...
    force_inline friend void bigint_add(bigint &o, bigint const &lhs, bigint const &rhs, T carry=0) noexcept
    {
        for (auto i = 0; i < N; i++) {
    ...
    force_inline friend void bigint_div(bigint &quotient, bigint &remainder, bigint const &lhs, bigint const &rhs) noexcept
    {
    ...
    force_inline friend void bigint_div(bigint &r_quotient, bigint &r_remainder, bigint const &lhs, bigint const &rhs, bigint<T,2*N> const &rhs_reciprocal) noexcept
    {
    ...
    */
    [[nodiscard]] force_inline friend int bigint_bsr(bigint const &rhs) noexcept
    {
    ...
    */
    [[nodiscard]] force_inline friend bigint bigint_reciprocal(bigint const &rhs) {
    ...
    force_inline friend void bigint_and(bigint &o, bigint const &lhs, bigint const &rhs) noexcept
    {
        for (auto i = 0; i != N; ++i) {
    ...
    force_inline friend void bigint_or(bigint &o, bigint const &lhs, bigint const &rhs) noexcept
    {
        for (auto i = 0; i != N; ++i) {
            o.digits[i] = lhs.digits[i] | rhs.digits[i];
        }
    ...
    force_inline friend void bigint_xor(bigint &o, bigint const &lhs, bigint const &rhs) noexcept
    {
        for (auto i = 0; i != N; ++i) {
            o.digits[i] = lhs.digits[i] ^ rhs.digits[i];
        }
    ...
    force_inline friend void bigint_subtract(bigint &o, bigint const &lhs, bigint const &rhs) noexcept
    {
        bigint rhs_;
    ...
    [[nodiscard]] force_inline friend bool operator==(bigint const &lhs, bigint const &rhs) noexcept
    {
    ...
    template<typename R, std::enable_if_t<std::is_integral_v<R>,int> = 0>
    [[nodiscard]] force_inline friend auto operator<<(bigint const &lhs, R const &rhs) noexcept {
        bigint o;
    ...
    [[nodiscard]] force_inline friend auto operator*(bigint const &lhs, bigint const &rhs) noexcept
    {
        auto o = bigint{0};
    ...
    [[nodiscard]] force_inline friend auto operator~(bigint const &rhs) noexcept
    {
        bigint o;
        bigint_invert(o, rhs);
        return o;
    ...
    [[nodiscard]] force_inline friend auto div(bigint const &lhs, bigint const &rhs) noexcept
    {
    ...
    [[nodiscard]] force_inline friend auto div(bigint const &lhs, bigint const &rhs, bigint<T,2*N> const &rhs_reciprocal) noexcept
    {
        auto quotient = bigint{0};
        auto remainder = bigint{0};
